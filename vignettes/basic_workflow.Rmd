...
groups by separate, and merge both alignment results at a later step. This way,
we reduce the  variability between the initially aligned samples, thus reducing
the number of corrections to be performed over that initial alignment.
...
regarding which samples belong to which group, which something that you most
probably have already built as part of your sampling process. The package
...
to each of these list separately, using `lapply`. For example, we could apply
...
you use `align_chromatograms2()`. The main difference of the function, is that
it sets a default seed (It uses `withr::local_seed()` for this, so do not worry,
the execution of the function does not change the status of the random number
generator outside of the function's environment) before performing an
alignment process, which guarantees that using a given set of parameters for
a given data set, you will always get the same alignment result.
...
[paper](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0198311), so please go and check it. The difference here, is that I changed
the name of the function parameters to more closely reflect the name of the 
alignment step the `GCalignR` algorithm uses them for.
...
but `align_chromatograms2()` has no default for the corresponding parameter
(`linear_shift_criteria`) to force you to be explicit with your alignment
criteria, as this is usually a very arbitrary criteria. My advice would be to
keep it as a very small value (a fair conservative approach could be to just 
set it as 0), on the end you should not be aligning samples that have a very
strong RT variability for a given compound (at least, under the considerations
of the workflow presented here). Here we will just take the original default
value of 0.02, as it seemed to work well for the data set during my initial
trials of the function. Some possible approaches to define a linear shift that
is more bounded to the specific data set, thus less arbitrary, would be to have
standard runs before and after running a set of samples and use the average
difference in RT between homologous peaks. This implies that you think in
advance the number of samples you can run between standard runs, without
separating them too much in time, and would not be possible if you want to 
analyze GC-MS runs from the past (re-running the samples might also not be a
viable option in some cases). Also, if you used an internal standard with
your samples, for absolute quantification, you could use the average RT
difference of the internal standard peak across samples. However, an internal
standard is not always used (sometimes is not possible), and once the sample has
been extracted there is no use to adding one to the extract. Therefore, it might
be just easier to stick to lower values and dare to play a bit with them to find
one that seems to produce few miss-alignments (you must probably will have some)
with your given data set.

In the case of the partial peak alignment, a very recommendable approach would
be to use `GCalignR::peak_interspace()` to get an idea of the minimum 
separation between neighboring (adjacent) peaks within samples, and use this value.
...
One possible approach would be to use `GCalignR::peak_interspace()` again, but
setting the quantile range to the upper part of the variance (i.e. from 0.6 to
0.9) and use the highest value, but in my experience is difficult to decide
within any set of arbitrary variance roof values for this range (e.g. between
0.9 and 0.95) as they might differ very strongly, and in my particular case
I could not really find a "sweet spot" to stay with while testing this approach.
If I was to use such approach I would probably just stick to the highest value
obtained by using a 0.6 to 0.9 quantile range.
Another approach could be to use the maximum distance between homologous peaks
between standard runs before and after our samples, or between the peaks of an
internal standard among samples, similar to the
`linear_shift_criteria`. But, as previously said, these approaches might not
always be possible. Therefore, taking advantage of my previous knowledge on the
data set we are using here, I decided to set the `row_merging_threshold` to 
0.15, which was the maximum separation I could find between peaks of any given
hydrocarbon among the samples. Again, this approach might only make sense 
(at least time-wise) with small data sets, and or data set about which we have 
a good amount of prior knowledge, and it would not be really reproducible.

On the end, the most important thing, when deciding for the alignment 
parameters, is to explicitly report them, as well as why and how did we decided 
to use them.

So, now that we have decided for the values of our alignment parameters, we can 
use `align_chromatograms2()` to perform the automatic alignment of our data.
...
cluster, using two of our processor cores, and then with this cluster we 
...
tend to contain miss-alignments).
...
frame, using proprietary software like MassHunter, can be very time
consuming, not reproducible, and prone to errors. 
...
alternative heat map plot, which displays the normalized abundance of the peaks
(%) for every sample. Additionally, the samples are ordered regarding their
similarity within the plot, via a hierarchical cluster that is displayed as a
dendrogram. For us, such plot has resulted considerably more useful for finding
possible miss-alignments, while it also offers a first overall view of the data
within each aligned group data set. As a result, this package includes a 
function `diagnostic_heatmap()` that produces this plot. 

We can normalize the abundance data in advance with `area_norm()`, which is a 
wrapper around `GCalignR::norm_peaks()`, before using `diagnostic_heatmap()`
to generate the alignment diagnostic plot. Alternatively, it is also possible 
to directly provide an aligned data set object to `diagnostic_heatmap()`, as
it calls `area_norm()`, if the provided data set has not yet been normalized.

Given the illustrative purposes of this guide, we will first normalize the 
data sets with `area_norm()`.

```{r}
samples_area_norm_list <- aligned_samples_data_list |>
  lapply(area_norm)

standards_area_norm <- area_norm(aligned_standards)
```

Now, we can generate the diagnostic plots with `diagnostic_heatmap()`.

_Note: The diagnostic plot tends to be very big, so I would normally recommend_
_you to use `pdf()` to generate the plots within a PDF document, where they_
_will be easier to explore. Given the illustrative purposes of this guide, I_
_am not doing that to be able show the plots here._ 
```{r eval=F, include=T}
...
diagnostic_heatmap(standards_area_norm
...
As you can see, the rows of the peaks break at samples 351 an 350, for the peaks
P106 and P107 respectively. These breaks look like stairs, which is why I call
them this way. So, now that we know the RT values of the peaks where there could
be a miss-alignment, we can check them within the chromatograms, and in the case
of GC-MS data, we could even check the mass spectra. By such process we can
verify if the rows forming a stair do actually correspond to the same peak, thus
being a miss-alignment.

# Correct the alignment
Once all the miss-alignments have been identified, it is necessary to correct
them. The miss-alignments correction consist on the displacement of peaks from
samples, to obtain a data frame where each row corresponds to the same 
peak, thus same compound(s), across samples.
