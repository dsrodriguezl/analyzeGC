...
regarding which samples belong to which group, which  most
...
would be to keep it as a very small value (a fair conservative approach could
...
original default value of 0.02, as it seemed to work well for the data set
during my initial trials of the function. Some possible approaches to define a
linear shift that is more bounded to the specific data set, thus less
...
standard runs, without separating them too much in time, and would not be
possible if you want to analyze GC-MS runs from the past (re-running the
samples might also not be a viable option in some cases). Also, if you used an
internal standard with your samples, for absolute quantification, you could use
the average RT difference of the internal standard peak across samples.
However, an internal standard is not always used (sometimes is not possible),
and once the sample has been extracted there is no use to adding one to the
extract. 
Therefore, it might be just easier to stick to lower values and dare to play a
bit with them to find one that seems to produce few miss-alignments (you must
probably will have some) with your given data set.
...
such data frames for you own data.
