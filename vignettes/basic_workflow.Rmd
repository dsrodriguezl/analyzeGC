...
However, if the values to displace represent a different peaks to those 
...
`position.reference` indicates a peak (e.g. P10) as a location reference
...
First, we need to create a list of data frames, where each entry name 
correspond to the label of any sample within the aligned group data to modify.
...
We can confirm that the empty rows where inserted in the right place of each
group data set, by looking at their RT data frames. 
```{r}
...
In the case of `correct_alignment()`, it allows the displacement of the Area 
and RT values of peaks, within samples, to correct the alignment.
