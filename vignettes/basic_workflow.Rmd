...
types of GC data (i.e. non-CHC  extracts). However, I have not been able to 
find a better alternative, in the case the first mentioned approach is not
applicable.
...
As you can see, the rows of the peaks break at samples 351 and 350, for the
peaks P106 and P107 respectively. These breaks look like stairs, which is why I
call them this way. So, now that we know the RT values of the peaks where there
could be a miss-alignment, we can check them within the chromatograms, and in
the case of GC-MS data, we could even check the mass spectra. 
By such process we can verify if the rows forming a stair do actually
...
## Add empty peaks
If the values within a row, corresponds to more than one peak, the miss-aligned
values should be displaced to the peak before or after.
However, if the values to displace represent different peaks to those 
represented by the rows around, then we need to insert an empty peak (row)
...
the empty peaks. In the list, each entry name corresponds to the label of a
sample (can be any) within the aligned group data to modify.
`add_empty_peaks()` will only create the empty row if the sample is in the
data frame it is modifying.
...
group data set,w e just need to look at their RT data frames.
...
upper row, then the values of the first peak should be displaced before other,
or the values of the latter peak will overwrite the values of the first.
...
removed from the aligned data sets, which would no longer be correct due tot he 
...
Now we have the corrected alignment of each group, but we do not now what 
...
The idea is to create a new table, modifying the exported RT data frames, that
will also contain the identification of the compounds contained by the 
different peaks.
...
```{r}
lapply(comps_id_list, str)

str(comps_id_std)
```

As you can see, these data frames are very similar to the original RT data 
frames, except for their second column, which is called "Compound". 
...
"Compound", the functions of the package will work as long as the data set have
them in the right format.
Compound holds the identification of the hydrocarbon(s) contained by each
peak.
